...
export const VuiChat = ({
  ...
  useEffect(() => {
    ...
    const hasBottomQuestionJustChanged =
      ...
      prevConversationRef.current.isBottomQuestionLoading !==
        Boolean(conversation[conversation.length - 1]?.isLoading);
    ...
    const shouldStickToBottom =
      ...
      (isScrolledToBottomRef.current && hasBottomQuestionJustChanged);
  ...
  const cycleChatStyle = () => {
    ...
    setChatStyle(
      currentIndex === CHAT_STYLE_ORDER.length - 1
        ? CHAT_STYLE_ORDER[0]
        : CHAT_STYLE_ORDER[currentIndex + 1]
    );
  };
