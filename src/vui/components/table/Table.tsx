...
import {
  Props as TableBulkActionProps,
  VuiTableBulkActions,
} from "./TableBulkActions";
...
export const VuiTable = <T extends Row>({
  ...
  const [rowBeingActedUpon, setRowBeingActedUpon] = useState<T | undefined>(
    undefined
  );
  ...
  const columnCount =
    columns.length + (onSelectRow ? 1 : 0) + (actions ? 1 : 0);
  ...
  if (content) {
    tableContent = (
      <VuiTableContent colSpan={columnCount}>{content}</VuiTableContent>
    );
  } else if (isLoading) {
    tableContent = (
              ...
              <VuiTableCell>
                <VuiCheckbox
                  ...
                  onChange={() => {
                    ...
                    onSelectRow(
                      Object.keys(selectedIds).map(
                        (id) => rows.find(() => rowId === id) as T
                      )
                    );
                  }}
                />
